...
/**
 * 该类只能处理integrator中中两个设备之间的harness路径为折线且只有一个bundle的情况，而且是一个harness下一个bundle。
...
*
 */
public class IDS_CAVAL_AutoWire_brokenline {
...
//         addNewBookToXMLDocument(document);
     ...
     * @param document
     */
    private static void ModifyInformationOfXMLDocument(List<Element> virtualharnessList)throws Exception {
...
//        		System.out.println(id.getValue());
        ...
        System.out.println("virtualharnessList.size():"+virtualharnessList.size());
        int count1 = 0;
        int count = 0;
        ...
        for (int i = 0; i < virtualharnessList.size(); i++) {
            ...
            List<Element> toponodeList = virtualharness.element("topoharness").elements("toponode");
            String node1X;
            String node2X;
            System.out.println("toponodeList.size()>>:"+toponodeList.size());
            if(toponodeList.size()==2){
            	count1++;
            	node1X = toponodeList.get(0).attribute("x").getValue();
            	node2X = toponodeList.get(1).attribute("x").getValue();
            	
            	float midX =(Float.parseFloat(node1X)+Float.parseFloat(node2X))/2; 
            	
            	String node1Y = toponodeList.get(0).attribute("y").getValue();
            	String node2Y = toponodeList.get(1).attribute("y").getValue();
            	
            	float midY =(Float.parseFloat(node1Y)+Float.parseFloat(node2Y))/2; 
            	
            	System.out.println(midX+"::"+midY);
            	//3、添加布线信息:添加topojoint和toposegment
            	Element topobundle = virtualharness.element("topoharness").element("topobundle");
            	Element toposegment = topobundle.element("toposegment");
            	String arrtibuteref = toposegment.attributeValue("attributeref");
            	if(topobundle.elements("topojoint").size()!=2){
            		...
            		toposegment1.addAttribute("start", toponodeList.get(0).attributeValue("id"));
            		toposegment1.addAttribute("end", topojoint1.attributeValue("id"));
            		...
            		toposegment2.addAttribute("start", topojoint1.attributeValue("id"));
            		toposegment2.addAttribute("end", topojoint2.attributeValue("id"));
            		toposegment2.addAttribute("baseid", uid2+time);
            		toposegment2.addAttribute("attributeref", arrtibuteref);
            		
            		toposegment.addAttribute("start", topojoint2.attributeValue("id"));
            		toposegment.addAttribute("end",toponodeList.get(1).attributeValue("id") );
    ...
    private static void traverseBooks(List<Element> virtualharnessList) {
        for (Iterator<Element> iterator = virtualharnessList.iterator(); iterator.hasNext();) {
     ...
     * @return
     */
    private static List<Element> readAllElementsFromXMLDocument(Document document) {
        Element root = document.getRootElement();
        List<Element> virtualharnessList = root.element("designmgr").element("topologydesign").element("planedesign").element("topoplane").elements("virtualharness");
   ...
   */
    ...
    private static void writeToNewXMLDocument(Document document)
        ...
        XMLWriter writer = new XMLWriter(new FileWriter(
                "D:/eclipse_wks/IDS_Test/IntegratorDesign2_20180329_brokenline.xml"));
//        XMLWriter writer = new XMLWriter(new FileWriter(
//                "D:/eclipse_wks/IDS_Test/integratorDesign1_update.xml"));
    ...
    private static Document getDocument() throws Exception {
        ...
        Document document = sr.read("D:/eclipse_wks/IDS_Test/IntegratorDesign2_20180329.xml");
//        Document document = sr.read("D:/eclipse_wks/IDS_Test/integratorDesign1.xml");
        return document;
    }
}
