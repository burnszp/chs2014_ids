...
/**
 * 每个harness涉及到的设备都通过一个点，连接起来，按照最短路径来链接。[目前仅仅把不和设备直接相连接的bundle删除。]
 * 
 ...
 * 
 */
public class IDS_CAVAL_AutoWire_20180411 {
	...
	private static void ModifyInformationOfXMLDocument(Element topoplane)
		...
		for (int i = 0; i < virtualharnessList.size(); i++) {
			...
			for (String node : notHasPropertyConnector_idNodeMap.keySet()) {
				System.out.print(node + " $ ");
			}
			// 判断每个virtualharness中的phd下的每个bundle，如果每个bundle下的startnoderef和endnoderef所指向的node都不是包含属性connector_id，则考虑把该bundle删除。
			Map<String, Element> bundleShouldDeleteMap = new HashMap<String, Element>();
			List<Element> bundleList = virtualharness.element("phd").elements(
					"bundle");
			...
			Map<String,Element> toponodeShouldDeleteMap = new HashMap<String,Element>();
			...
			for (Element toponode : toponodeList) {
				...
				if(notHasPropertyConnector_idNodeMap.get(connref)!=null){
					toponodeShouldDeleteMap.put(toponode.attributeValue("id"), toponode);
				}
			}
			
			Map<String,Element> topobundleShouldDeleteMap = new HashMap<String,Element>();
			...
			for (Element topobundle : topobundleList) {
				...
				if(bundleShouldDeleteMap.get(connref)!=null){
					topobundleShouldDeleteMap.put(topobundle.attributeValue("id"), topobundle);
				}
			}
...
//			}
	...
	private static void writeToNewXMLDocument(Document document)
		...
		XMLWriter writer = new XMLWriter(
				new FileWriter(
						"D:/eclipse_wks/IDS_Test/20180411/IntegratorDesign4_1.xml"));
	...
	private static Document getDocument() throws Exception {
		...
		Document document = sr
				.read("D:/eclipse_wks/IDS_Test/20180411/IntegratorDesign4.xml");
		// Document document =
